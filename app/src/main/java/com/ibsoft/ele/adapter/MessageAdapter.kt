...
class MessageAdapter(
    ...
    inner class UserMessageViewHolder(val binding: ItemMessageUserBinding)
        ...
        fun bind(message: Message) {
            ...
            // Set up the copy button to copy the message text.
    ...
    override fun getItemViewType(position: Int): Int {
        return if (messages[position].sender.equals("user", ignoreCase = true)) TYPE_USER else TYPE_BOT
    }
                ...
                if (seg.isCode) {
                    inSpans(android.text.style.TypefaceSpan("monospace")) {
                        ...
                        matches.sortedBy { it.range.first }.forEach { match ->
                            ...
                            color(segmentColor) { append(match.value) }
